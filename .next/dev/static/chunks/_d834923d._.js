...
_s(NavigationTabsContent, "AxA9T5G2Po78UC4hL8ljCdvMciE=", false, function() {
    return [
...
function PlatformList({ category }) {
    ...
    (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$index$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["useEffect"])({
        "PlatformList.useEffect": ()=>{
            const connectedPlatforms = loadConnectedPlatforms();
            const categoryPlatforms = samplePlatforms.filter({
                "PlatformList.useEffect.categoryPlatforms": (p)=>p.category === category
            }["PlatformList.useEffect.categoryPlatforms"]).map({
                "PlatformList.useEffect.categoryPlatforms": (p)=>({
                        ...p,
                        connected: connectedPlatforms[p.id]?.connected || false,
                        apiKey: connectedPlatforms[p.id]?.apiKey
                    })
            }["PlatformList.useEffect.categoryPlatforms"]);
...
_s(PlatformList, "mhfnElJkalMrv0ZuYdhYwUQ16yU=");
_c = PlatformList;
